...
<!-- Navigation--> 
	...
	<br><br>
	<div style="width: 50%; border: 1px solid black;" id="crew">
	...
	<br>
	<form method="post" action="<%=request.getContextPath() %>/crew/crewInsert.do">
		<input type="hidden" name="crewCode" value="<%=rcv.getCrewCode() %>">
		<input type="hidden" name="memCode" value="<%=memVO.getMemCode() %>">
		<%
			if(add <= 0){
		%>
		<button type="submit" disabled="disabled">크루 가입</button>
		<%		
			}else{
		%>
		<button type="submit" onclick="proc()">크루 가입</button>		
		<%	
			}
		 %>
	</form>
